...
download_podcasts() {
    feed_url=$1
    folder_name=$2
    max_episodes=${3:-9999999}

    if [ -z "$feed_url" ] || [ -z "$folder_name" ]; then
        echo "Error: Feed URL and folder name must be specified."
        return
    fi
    ...
    for ((i = 0; i < ${#url_list[@]} && i < max_episodes; i++)); do
        url="${url_list[i]}"
        title="${title_list[i]}"
        guid="${guid_list[i]}"
        ...
        if grep -qFx "$feed_url • $guid" podarchive.txt; then
            ...
            if curl -s "$url" | pv -s $(curl -sI "$url" | grep -i Content-Length | awk '{print $2}') -N "$title" | tar -xzf - -C "$folder_path/$title.$file_extension"; then
...
read -p "Enter the folder name where files will be saved: " folder_name
